...
def build_table(count: int, minimum_r: float, maximum_r: float, dps: int):
    ...
    h = log_r_mp[1] - log_r_mp[0]
    coefficient_rows = []
    for profile in (0, 3, 6):
        coefficients = [[] for _ in range(6)]
        for index in range(count - 1):
            a0 = rows[profile][index]
            a1 = h * rows[profile + 1][index]
            a2 = mp.mpf("0.5") * h**2 * rows[profile + 2][index]
            f = rows[profile][index + 1] - (a0 + a1 + a2)
            g = h * rows[profile + 1][index + 1] - (a1 + 2 * a2)
            curvature = h**2 * rows[profile + 2][index + 1] - 2 * a2
            interval = (a0, a1, a2, 10 * f - 4 * g + curvature / 2,
                        -15 * f + 7 * g - curvature,
                        6 * f - 3 * g + curvature / 2)
            for row, value in zip(coefficients, interval):
                row.append(value)
        for row in coefficients:
            row.append(mp.mpf(0))
        coefficient_rows.extend(coefficients)
    all_rows = rows + coefficient_rows
    return (np.array([float(value) for value in log_r_mp]),
            np.array([[float(value) for value in row] for row in all_rows]),
...
def write_header(path: Path, count: int, minimum_r: float,
    ...
    names = ("kTrumpetAlpha", "kTrumpetAlphaDy", "kTrumpetAlphaDyy",
             ...
             "kTrumpetAlphaA0", "kTrumpetAlphaA1", "kTrumpetAlphaA2",
             "kTrumpetAlphaA3", "kTrumpetAlphaA4", "kTrumpetAlphaA5",
             "kTrumpetArealRadiusA0", "kTrumpetArealRadiusA1",
             "kTrumpetArealRadiusA2", "kTrumpetArealRadiusA3",
             "kTrumpetArealRadiusA4", "kTrumpetArealRadiusA5",
             "kTrumpetShiftQA0", "kTrumpetShiftQA1", "kTrumpetShiftQA2",
             "kTrumpetShiftQA3", "kTrumpetShiftQA4", "kTrumpetShiftQA5")
