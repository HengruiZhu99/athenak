...
//        pmy_pack(pp), ps{&eos} {
    ...
    ps.GetEOSMutable().SetDensityFloor(
        pin->GetOrAddReal(block, "dfloor", (FLT_MIN))/mb);

    // A temperature floor applies at every density.  Do not infer it from the
    // atmosphere ratio pfloor/dfloor: doing so heats any finite-density fluid
    // whose physical temperature lies below that atmosphere ratio.  Problem
    // generators may initialize atmosphere pressure independently through
    // pfloor; a global temperature floor must be explicitly requested.
    ps.GetEOSMutable().SetTemperatureFloor(
        pin->GetOrAddReal(block, "tfloor", (FLT_MIN)));
