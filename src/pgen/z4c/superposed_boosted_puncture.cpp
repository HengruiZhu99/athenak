...
AthenaScratchTensor<Real, TensorSymm::SYM2, 4, 2> inverse(
    AthenaScratchTensor<Real, TensorSymm::SYM2, 4, 2> matrix);
...
void AddBoostedPuncture(MeshBlockPack *pmbp, ParameterInput *pin, int punc_num) {
  ...
  int nmb = pmbp->nmb_thispack;
  Real m0 =
      pin->GetOrAddReal("problem", "punc_" + std::to_string(punc_num) + "_rest_mass", 1.);
  Real center_x1 =
      pin->GetOrAddReal("problem", "punc_" + std::to_string(punc_num) + "_center_x1", 0.);
  Real center_x2 =
      pin->GetOrAddReal("problem", "punc_" + std::to_string(punc_num) + "_center_x2", 0.);
  Real center_x3 =
      pin->GetOrAddReal("problem", "punc_" + std::to_string(punc_num) + "_center_x3", 0.);
  Real vel =
      pin->GetOrAddReal("problem", "punc_" + std::to_string(punc_num) + "_velocity_x1",
                        0.);  // Example velocity
  // Lorentz factor
