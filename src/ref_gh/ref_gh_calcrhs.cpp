...
struct AnalyticPrimaryTeamScratch {
  AnalyticRadialQPoint reference;
  ...
  CompactAnalyticSourceWorkspace source_workspace;
  Real gauge_coordinate_extra[4][4]; // NOLINT(runtime/arrays)
...
template <int FDNG, int A, int B>
KOKKOS_INLINE_FUNCTION
void StoreAnalyticPrimaryTeamComponent(
    const DvceArray5D<Real> &state, const DvceArray5D<Real> &state_rhs,
    const int m, const int k, const int j, const int i, const Real idx[3],
    const Real gamma0, const Real gamma2, const int phi_ordering,
    const bool gauge_driver_enabled,
    const AnalyticPrimaryTeamScratch &scratch) {
  Real scalar_source = CompactAnalyticRadialQScalarWaveSourceComponent<A, B>(
      scratch.psi, scratch.reference, gamma0, scratch.source_workspace);
  if (gauge_driver_enabled) {
    for (int a = 0; a < 4; ++a) {
      for (int b = 0; b < 4; ++b) {
        scalar_source += ReferenceFrame(scratch.reference, A, a)
                         *ReferenceFrame(scratch.reference, B, b)
                         *scratch.gauge_coordinate_extra[a][b];
      }
    }
  }
  ...
  for (int p = 0; p < 3; ++p) {
    for (int q = 0; q < 3; ++q) {
      ...
      for (int I = 0; I < 3; ++I) {
        partial_tilde_phi +=
            CoframeDerivative(scratch.reference, p + 1, I + 1, q + 1)
              *scratch.phi[I][A][B]
            + ReferenceSpatialCoframe(scratch.reference, I, q)
              *Dx<FDNG>(p, idx, state, m, PhiIndex(I, A, B), k, j, i);
        tilde_phi_q += ReferenceSpatialCoframe(scratch.reference, I, q)
                       *scratch.phi[I][A][B];
      }
      ...
      for (int r = 0; r < 3; ++r) {
        ...
        for (int I = 0; I < 3; ++I) {
          tilde_phi_r += ReferenceSpatialCoframe(scratch.reference, I, r)
                         *scratch.phi[I][A][B];
        }
        covariant_derivative -= scratch.spatial_connection[r][p][q]
                                *tilde_phi_r;
      }
      divergence += scratch.spatial_inverse[p][q]*covariant_derivative;
      lapse_gradient_term += scratch.spatial_inverse[p][q]
                             *scratch.d_alpha[p]*tilde_phi_q;
    }
  }
  const CoordinateGhGeometry &geometry = scratch.compact_geometry.geometry;
  Real pi_rhs = geometry.lapse*(scratch.trace_k*scratch.pi[A][B] - divergence
                                 + scalar_source)
                - lapse_gradient_term;
  for (int p = 0; p < 3; ++p) {
  ...
  if (phi_ordering == 0) {
    for (int I = 0; I < 3; ++I) {
      Real phi_rhs = 0.0;
      for (int p = 0; p < 3; ++p) {
        phi_rhs += ReferenceSpatialFrame(scratch.reference, I, p)
                   *Dx<FDNG>(p, idx, state_rhs, m, PsiIndex(A, B), k, j, i);
        Real coordinate_d_psi = 0.0;
        for (int J = 0; J < 3; ++J) {
          coordinate_d_psi += ReferenceSpatialCoframe(scratch.reference, J, p)
                              *scratch.phi[J][A][B];
        }
        phi_rhs += ReferenceDtSpatialFrame(scratch.reference, I, p)
                   *coordinate_d_psi;
      }
      state_rhs(m, PhiIndex(I, A, B), k, j, i) = phi_rhs;
    }
  } else {
    Real beta_frame[3] = {};  // NOLINT(runtime/arrays)
    for (int J = 0; J < 3; ++J) {
      for (int p = 0; p < 3; ++p) {
        beta_frame[J] += ReferenceSpatialCoframe(scratch.reference, J, p)
                         *geometry.shift[p];
      }
    }
    ...
    for (int I = 0; I < 3; ++I) {
      for (int J = 0; J < 3; ++J) {
        for (int K = 0; K < 3; ++K) {
          structure[I][J][K] =
              ReferenceStructure(scratch.reference, I, J, K);
        }
      }
    }
    Real phi_value[3];                 // NOLINT(runtime/arrays)
    Real coordinate_d_phi[3][3];      // NOLINT(runtime/arrays)
    Real frame_derivative[3][3];      // NOLINT(runtime/arrays)
    for (int J = 0; J < 3; ++J) {
      phi_value[J] = scratch.phi[J][A][B];
      for (int p = 0; p < 3; ++p) {
        coordinate_d_phi[J][p] = Dx<FDNG>(
            p, idx, state, m, PhiIndex(J, A, B), k, j, i);
      }
    }
    for (int I = 0; I < 3; ++I) {
      for (int J = 0; J < 3; ++J) {
        ...
        for (int p = 0; p < 3; ++p) {
          frame_derivative[I][J] +=
              ReferenceSpatialFrame(scratch.reference, I, p)
    ...
    for (int I = 0; I < 3; ++I) {
      ...
      phi_rhs -= StandardPhiOrderingCorrection(
          I, beta_frame, frame_derivative, structure, phi_value);
      state_rhs(m, PhiIndex(I, A, B), k, j, i) = phi_rhs;
    }
  }
  ...
  if (gamma2 > 0.0) {
    ...
    for (int p = 0; p < 3; ++p) {
      coordinate_reduction[p] =
          Dx<FDNG>(p, idx, state, m, PsiIndex(A, B), k, j, i);
      for (int I = 0; I < 3; ++I) {
        coordinate_reduction[p] -=
            ReferenceSpatialCoframe(scratch.reference, I, p)
            *scratch.phi[I][A][B];
        spatial_frame[I][p] =
            ReferenceSpatialFrame(scratch.reference, I, p);
      }
    ...
    const Gamma2DampingRhs damping = ComputeGamma2DampingRhs(
        geometry.lapse, geometry.shift, coordinate_reduction, spatial_frame,
        gamma2);
    state_rhs(m, PiIndex(A, B), k, j, i) += damping.pi;
    for (int I = 0; I < 3; ++I) {
      state_rhs(m, PhiIndex(I, A, B), k, j, i) += damping.phi[I];
    }
  }
}
...
template <int FDNG>
KOKKOS_INLINE_FUNCTION
void DispatchAnalyticPrimaryTeamComponent(
    const int component, const DvceArray5D<Real> &state,
    const DvceArray5D<Real> &state_rhs, const int m, const int k,
    const int j, const int i, const Real idx[3], const Real gamma0,
    const Real gamma2, const int phi_ordering,
    const bool gauge_driver_enabled,
    const AnalyticPrimaryTeamScratch &scratch) {
  switch (component) {
    case 0: StoreAnalyticPrimaryTeamComponent<FDNG, 0, 0>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 1: StoreAnalyticPrimaryTeamComponent<FDNG, 0, 1>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 2: StoreAnalyticPrimaryTeamComponent<FDNG, 0, 2>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 3: StoreAnalyticPrimaryTeamComponent<FDNG, 0, 3>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 4: StoreAnalyticPrimaryTeamComponent<FDNG, 1, 1>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 5: StoreAnalyticPrimaryTeamComponent<FDNG, 1, 2>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 6: StoreAnalyticPrimaryTeamComponent<FDNG, 1, 3>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 7: StoreAnalyticPrimaryTeamComponent<FDNG, 2, 2>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 8: StoreAnalyticPrimaryTeamComponent<FDNG, 2, 3>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
    case 9: StoreAnalyticPrimaryTeamComponent<FDNG, 3, 3>(
        state, state_rhs, m, k, j, i, idx, gamma0, gamma2, phi_ordering,
        gauge_driver_enabled,
        scratch); break;
  }
...
TaskStatus RefGh::CalcRHSImpl(Driver *driver, int stage) {
  ...
  if constexpr (Analytic) {
    const int ni = indcs.ie - indcs.is + 1;
    const int nj = indcs.je - indcs.js + 1;
    ...
    KOKKOS_LAMBDA(const TeamMember_t &team) {
      ...
      const int m = league/nkji;
      const int local = league - m*nkji;
      const int k = local/nji + indcs.ks;
      const int j = (local - (k - indcs.ks)*nji)/ni + indcs.js;
      const int i = local%ni + indcs.is;
      auto *scratch = static_cast<AnalyticPrimaryTeamScratch *>(
          team.team_shmem().get_shmem(sizeof(AnalyticPrimaryTeamScratch)));
      Kokkos::single(Kokkos::PerTeam(team), [&]() {
        ...
        const Real z = CellCenterX(k - indcs.ks, indcs.nx3,
                                   size.d_view(m).x3min, size.d_view(m).x3max);
        scratch->reference = MakeTypedProductionReferencePoint<true>(
            reference_cache, reference_extra, analytic_static, analytic_stage,
            m, k, j, i, x, y, z, center_x, center_y, center_z);
        Real determinant = 0.0;
        if (!LoadCompactAnalyticPointGeometry(
                state, scratch->reference, m, k, j, i, scratch->psi,
                scratch->pi, scratch->phi, scratch->d_psi, scratch->metric,
                scratch->d_metric, scratch->compact_geometry, determinant)) {
          scratch->valid = 0;
        }
        const Real idx[3] = {1.0/size.d_view(m).dx1,
                             1.0/size.d_view(m).dx2,
                             1.0/size.d_view(m).dx3};
        if (scratch->valid && gauge_driver_enabled) {
          Real hhat[4];       // NOLINT(runtime/arrays)
          Real theta[4];      // NOLINT(runtime/arrays)
          Real upsilon[3];    // NOLINT(runtime/arrays)
          ...
          ReferenceGaugeBaseline baseline{};
          if (gauge_reference_subtraction) {
            baseline = ComputeProductionReferenceGaugeBaseline(
                scratch->reference);
            if (!baseline.valid) scratch->valid = 0;
          }
          if (scratch->valid) {
            for (int A = 0; A < 4; ++A) {
              hhat[A] = state(m, kHhatOffset + A, k, j, i)
            ...
            const CoordinateGhGeometry &geometry =
                scratch->compact_geometry.geometry;
            if (!ComputePhysicalGaugeTarget(
                    scratch->metric, scratch->d_metric, geometry,
                    scratch->reference, upsilon, shift_nu, shift_eta,
                    target)) {
              scratch->valid = 0;
            } else {
              const GaugeDriverRhs gauge_rhs = ComputeGaugeDriverRhs(
                  scratch->reference, hhat, theta, upsilon, d_hhat + 1,
                ...
                state_rhs(m, kThetaOffset + A, k, j, i) = gauge_rhs.theta[A]
                    - ((gauge_reference_subtraction
                        && reference_time_dependent)
                           ? ProductionReferenceDtTheta(scratch->reference, A)
                           : 0.0);
                d_hhat[0][A] = gauge_rhs.hhat[A];
              }
              for (int p = 0; p < 3; ++p) {
                state_rhs(m, kUpsilonOffset + p, k, j, i) =
                    gauge_rhs.upsilon[p];
              }
              ComputeCompactAnalyticOrdinaryGaugeCoordinateExtra(
                  scratch->metric, scratch->d_metric, scratch->reference,
                  scratch->compact_geometry, hhat, d_hhat, gamma0,
                  scratch->gauge_coordinate_extra);
            }
          }
          if (!scratch->valid) {
            for (int n = kHhatOffset; n < nvar; ++n) {
        ...
        if (scratch->valid
            && !PrepareCompactAnalyticRadialQScalarWaveSource(
                scratch->psi, scratch->pi, scratch->phi,
                scratch->reference, scratch->compact_geometry.geometry,
                scratch->source_workspace)) {
          scratch->valid = 0;
        }
        Real spatial_determinant = 0.0;
        if (scratch->valid
            && !InvertSpatial3(scratch->metric, scratch->spatial_inverse,
                               spatial_determinant)) {
          scratch->valid = 0;
        }
        if (scratch->valid) {
          for (int q = 0; q < 3; ++q) {
            for (int p = 0; p < 3; ++p) {
              for (int r = 0; r < 3; ++r) {
                scratch->spatial_connection[q][p][r] = 0.0;
                for (int ell = 0; ell < 3; ++ell) {
                  scratch->spatial_connection[q][p][r] +=
                      0.5*scratch->spatial_inverse[q][ell]
                      *(scratch->d_metric[p + 1][ell + 1][r + 1]
                        + scratch->d_metric[r + 1][ell + 1][p + 1]
                        - scratch->d_metric[ell + 1][p + 1][r + 1]);
                }
              }
            }
          }
          ...
          for (int p = 0; p < 3; ++p) {
            for (int q = 0; q < 3; ++q) {
              scratch->trace_k -= geometry.lapse
                                  *scratch->spatial_inverse[p][q]
            ...
            for (int a = 0; a < 4; ++a) {
              for (int b = 0; b < 4; ++b) {
                d_inverse_00 -= geometry.inverse_metric[0][a]
                                ...
                                *scratch->d_metric[p + 1][a][b];
              }
            }
            scratch->d_alpha[p] = 0.5*geometry.lapse*geometry.lapse
                                   *geometry.lapse*d_inverse_00;
          }
        }
      });
      team.team_barrier();
      const Real idx[3] = {1.0/size.d_view(m).dx1,
                           ...
                           1.0/size.d_view(m).dx3};
      Kokkos::parallel_for(
          Kokkos::TeamThreadRange(team, kSymmetric4Size),
          [&](const int component) {
            if (scratch->valid) {
              DispatchAnalyticPrimaryTeamComponent<FDNG>(
                  component, state, state_rhs, m, k, j, i, idx, gamma0,
                  gamma2, phi_ordering, gauge_driver_enabled, *scratch);
            } else {
              state_rhs(m, kPiOffset + component, k, j, i) = NAN;
            }
          });
    });
  }
