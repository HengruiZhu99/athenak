...
// Experimental physical-constraint radiation residual for differential CPBC.
// Shifted-trumpet perturbation tests fail with this candidate. Exact equilibrium
// preservation is not a stability guarantee; see analysis/tde_stability/
// BOUNDARY_RADIATION.md. This option is retained for controlled diagnostics.
...
int ComputeConstraintRadiationResidual(
  ...
  if (!(isfinite(r) && isfinite(R) && isfinite(alpha) && isfinite(chi) &&
        isfinite(chi_bg)) || r <= 0.0 || R <= 0.0 || alpha <= 0.0 ||
  ...
  f_theta = rhs(m,Z4c::I_Z4C_THETA,k,j,i)+omega*theta;
  for (int a = 0; a < 3; ++a) f_theta += transport[a]*dtheta[a];

  Real f_z[3];
  for (int a = 0; a < 3; ++a) {
    f_z[a] = z_t[a]+omega*z[a];
    for (int d = 0; d < 3; ++d) {
