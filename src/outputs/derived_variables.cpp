...
void ComputeZ4cDerivedDiagnostics(DvceArray5D<Real> derived, Mesh *pm) {
  ...
  par_for(
      ...
      KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
        ...
        derived(m, 0, k, j, i) = diagnostic.kretschmann;
        for (int component = 0; component < 6; ++component) {
          derived(m, 1 + component, k, j, i) = diagnostic.electric[component];
          derived(m, 7 + component, k, j, i) = diagnostic.magnetic[component];
        }
        for (int component = 0; component < 3; ++component) {
          derived(m, 13 + component, k, j, i) = diagnostic.poynting[component];
        }
      });
}
