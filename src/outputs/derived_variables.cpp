...
// get all moments terms for v_i and B_i
  ...
  if (name.compare("prtcl_d") == 0) {
    ...
    par_for(
        "pdens", DevExeSpace(), 0, (npart - 1), KOKKOS_LAMBDA(const int p) {
              ...
              for (int l1 = 0; l1 < 3; ++l1) {
                for (int m1 = 0; m1 < 3; ++m1) {
                  for (int n1 = 0; n1 < 3; ++n1) {
                    eps_duu[i1][k1][l1] +=
                        g_uu(k1, m1) * g_uu(l1, n1) * LC[i1][m1][n1] * sqrt_g;
                  }
                }
              }
            }
          ...
          for (int a = 0; a < 3; ++a) {
            for (int b = 0; b < 3; ++b) {
              for (int c = 0; c < 3; ++c) {
                for (int d = 0; d < 3; ++d) {
                  E_uu[a][b] +=
                      g_uu(a, c) * g_uu(b, d) * E_dd(std::min(c, d), std::max(c, d));
                  B_uu[a][b] +=
                      g_uu(a, c) * g_uu(b, d) * B_dd(std::min(c, d), std::max(c, d));
                }
              }
            }
          }
          ...
          for (int a = 0; a < 3; ++a) {
            for (int b = 0; b < 3; ++b) {
              for (int c = 0; c < 3; ++c) {
                for (int m1 = 0; m1 < 3; ++m1) {
                  for (int l1 = 0; l1 < 3; ++l1) {
                    P_u(a) -= eps_uuu[a][b][c] * E_dd(std::min(b, m1), std::max(b, m1)) *
                              g_uu(m1, l1) * B_dd(std::min(c, l1), std::max(c, l1));
                  }
                }
              }
            }
