...
The 171-name generated manifest classifies 137 truncating series, 12 exact-zero
SO(2) identities, 12 nonzero exact plane-algebra identities, and 10 exact-discrete
dissipation comparisons.  Only truncating
...
Roundoff saturation is decided by a checksum-bound binary64 coefficient audit,
not by the observed error.  The generated inventory fixes the field maxima, the
$C_1,C_2,C_M,C_U,C_{\rm KO},C_E,C_O,C_Q$ row norms, operation caps,
analytic-oracle bounds, and one source-family row for every result.  The driver
evaluates these bounds in outward-rounded arithmetic over the one-dimensional
radial mask ($O(171N_\rho)$ work; axial multiplicity is analytic), aggregates
them with the same uniform or positive-$\rho$ cylindrical weights as the error,
and uses $\max(F_{\rm clean},8\Delta)$ for a noisy lane.  Once a resolution is
below its floor, every finer point is an absorbing inconclusive suffix; it cannot
create a passing ratio or re-enter after saturation.
...
eight.  Both explicitly require \texttt{Athena\_SINGLE\_PRECISION=OFF}; this
qualification is Real64 and does not generalize its tolerances to float.  The CPU
cache requires Kokkos Serial on/CUDA off.  The CUDA cache requires
