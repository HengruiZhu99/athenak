...
The historical `characteristic_cpbc` / `zero_rate` closure enforces zero **weighted RHS rate**, not zero incoming state. At constant linearization coefficients it therefore preserves an initially nonzero incoming characteristic trace. The direct-Theta Gaussian has a nonzero tail at the boundary. This is a concrete alternative explanation for a tiny late boundary plateau; it is not proof of the cause of the still-uncollected final C/D plateaus, and it does not invalidate earlier independently verified unstable roots.

The eighth-rank local preflight checkpoints demonstrate the retention at a fixed first-active +x face cell. Late GPU C/D checkpoint traces and refreshed final propagation plots remain uncollected because the known Aurora SSH control socket is absent and the `aurora` alias no longer resolves locally. A different live socket was identified as Perlmutter, not Aurora; no diagnostics were collected from it. Existing plots remain the last validated 373 snapshots through23808M from job8842248. Do not label those plots as the completed50000M run.
...
## What would discriminate the late plateau

1. Read C/D rank7 checkpoints at 0, 10k, 20k, 30k, 40k, 50k and final walltime, using `checkpoint_face_trace.py`. Compare C1, C2, the longitudinal gauge trace and every term at the same cell, rather than following a moving maximum. The separate all-rank checker must still verify the checkpoint cohorts and ghosts. Compare both amplitude-normalized curves and absolute drift floors.
2. Repeat the pulse with initial incoming data compatible with homogeneous boundary data. One controlled design keeps the inner Gaussian unchanged and smoothly tapers its initial tail between1024 and1536M, leaving all stencils at the physical boundary initially zero. This is a different initial-data test, not zeroing any evolved perturbation. A narrower Gaussian also removes the boundary tail approximately but changes the excited wavelengths more strongly.
...
`checkpoint_face_trace.py RUN OUTPUT --prefix PREFIX --indices 0 10 20 30 40 50 51` reads one fixed rank per cohort and emits all terms and file hashes. It asserts the uniform cube geometry (global 32³ or 64³, 8 blocks), M=0, adapted gauge and G=1; seed parameters and the fixed face coordinates come from the input/header. Missing requested files are listed explicitly.
