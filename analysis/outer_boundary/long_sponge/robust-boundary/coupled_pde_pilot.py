...
n=32;h=32.;angle=np.pi/8;N=2048;times=[0,500,2000,5000,10000,20000]
...
for dt in [8.,4.]:
 ...
 run={'dt':dt,'records':records};out['runs'].append(run);out['seconds']=time.monotonic()-tick;(p/'coupled-pde-pilot.json').write_text(json.dumps(out,indent=2)+'\n')
 np.savez(p/f'coupled-pde-final-dt{dt:g}.npz',u=u,initial=u0,h=h,mode=mode)
