...
def full_matrix(n=16,h=.25,angle_y=0.,angle_z=0.,degree=3,inner=4,outer=2,
                ...
                paper_lapse_factor=1.,paper_actual_d0=False,shift=1.,lapse_damping=.1,sponge_rate=0.,sponge_cells=8.,kappa=.1,sponge_order="before",sponge_end_cells=0.,kappa_taper_cells=0.,kappa_taper_end_cells=0.,eta=2.):
    ...
    if kappa_taper_cells>0:
        assert 0<=kappa_taper_end_cells<kappa_taper_cells
        kt=np.clip((kappa_taper_cells-depth)/(kappa_taper_cells-kappa_taper_end_cells),0,1)
        kap=kappa*(1-kt**3*(10-15*kt+6*kt*kt))[:,None]
    ...
    for i in range(3):
        ...
        if damping:g-=2*kap*alpha*(G[i]-sum(d(j,H[i][j]) for j in range(3)))
        put(names[13+i],g);put(names[17+i],shift*G[i]-(eta*B[i] if damping else 0))
    if damping:
        V[6*n:7*n]+=kap*alpha*theta;V[7*n:8*n]-=2*kap*alpha*theta
    ...
    Vp=V.copy()
    if damping:
        Vp[6*n:7*n]-=kap*alpha*theta;Vp[7*n:8*n]+=2*kap*alpha*theta
        for i in range(3):
            Vp[(13+i)*n:(14+i)*n]+=2*kap*alpha*(G[i]-sum(d(j,H[i][j]) for j in range(3)))
            Vp[(17+i)*n:(18+i)*n]+=eta*B[i]
    ...
    assert 0<=sponge_end_cells<sponge_cells
    ss=np.clip((sponge_cells-depth)/(sponge_cells-sponge_end_cells),0,1)
    sponge=sponge_rate*ss**3*(10-15*ss+6*ss*ss)
    if sponge_order not in ['before','after']:
        raise ValueError(sponge_order)
    if sponge_order=='before':
    ...
    for pos,s in [(0,-1),(n-1,1)]:
        ...
        if mode=='direct':
            sigma=kappa*alpha if damping else 0.
        ...
        L[12*n+pos]+=dcross
    if sponge_order=='after':
        for j in range(20):L[j*n:(j+1)*n]-=sponge[:,None]*S[j]
