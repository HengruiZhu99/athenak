...
assert json.loads((ROOT/'stages-v2/results.json').read_text())['checks']['zero_exact_including_ghosts']
base=(ROOT.parent/'stability-modes-20260919/evolution/small_dx025.athinput').read_text()
cases=[('pulse_cubic',1e-8,4,2,50),('pulse_linear',1e-8,2,2,50),('pulse_second',1e-8,2,2,50),('pulse_oneblock',1e-8,2,2,50)]
if len(sys.argv)>1:cases=[c for c in cases if c[0] in sys.argv[1:]]
assert cases
def run(case):
    name,amplitude,order,radius,target=case
    folder=ROOT/'pilots-v2'/name;folder.mkdir(parents=True,exist_ok=False)
    text=base
    ...
    if name == 'pulse_second':text=setv(text,'mesh','nghost',2)
    if name == 'pulse_oneblock':
        for axis in (1,2,3):text=setv(text,'meshblock',f'nx{axis}',int(8*radius))
