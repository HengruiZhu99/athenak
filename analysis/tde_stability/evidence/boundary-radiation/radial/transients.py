...
def initial(r):
 ...
 z=(r-.2)*(4-r)/(1.9**2);zp=(4.2-2*r)/(1.9**2);zpp=-2/1.9**2;f=z**8;fp=8*z**7*zp;fpp=56*z**6*zp**2+8*z**7*zpp
...
def run(n):
 ...
 o={'degree':n,'initial_profile':'Polynomial [((r-.2)*(4-r))/1.9^2]^8; zero through seventh derivative at endpoints; unit linear amplitude. Times use dense matrix exponential, no time-discretization error apart from floating point.','states':hist,'wall_seconds':time.monotonic()-start}
 (ROOT/f'transient-compatible-n{n}.json').write_text(json.dumps(o,indent=2)+'\n');print(n,'seconds',o['wall_seconds'],'final',[(k,v[-1]['constraint_L2_grid_over_initial_state']) for k,v in hist.items()],flush=True)
 return o
