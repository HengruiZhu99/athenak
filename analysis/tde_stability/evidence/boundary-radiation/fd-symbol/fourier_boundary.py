...
"""
import numpy as np
from fd_boundary import operators,active_derivative,char
...
def full_matrix(n=16,h=.25,angle_y=0.,angle_z=0.,degree=3,inner=4,outer=2,
                mode='radiation',tau=1.,diss=.5,alpha=1.,chi=1.,damping=False,
                beta=(0.,0.,0.),gauge_target='zero_rate',tt_target='zero_rate',
                paper_lapse_factor=1.,paper_actual_d0=False):
    D,Dxx,adv,KO,E=operators(n,h,degree,beta[0],diss,4)
    eye=np.eye(n);total=20*n
    S=[np.eye(total,dtype=complex)[j*n:(j+1)*n] for j in range(20)]
    ...
    for i in range(3):
        ...
        put(names[13+i],g);put(names[17+i],2*G[i]-(2*B[i] if damping else 0))
    if damping:
    ...
    for j in range(3):Xp[17+j]=normal_adv(B[j])+2*d(0,G[j])
    ...
    for pos,s in [(0,-1),(n-1,1)]:
        left,_=char.scalar_left(alpha,chi,2*alpha,2,1);lp=left[:,:4];ld=left[:,4:]
        ...
        if gauge_target=='tangential':
            ...
            lambdas=np.array([s*beta[0]+np.sqrt(2*alpha*chi),s*beta[0]+np.sqrt(8/3),lam,lam])
        ...
        for ai,gi,hi,bi,component in [(10,14,3,18,1),(11,15,4,19,2)]:
            gauge=np.sqrt(2)*V[gi*n+pos]+s*Db[pos]@V[bi*n:(bi+1)*n]
            gt=0.
            if gauge_target=='tangential':
                fullrate=np.sqrt(2)*Vp[gi*n+pos]+s*Xp[bi][pos]
                normalrate=(s*beta[0]+np.sqrt(2))*(np.sqrt(2)*s*D[pos]@S[gi]+Dxx[pos]@S[bi])
                gt=fullrate-normalrate
            dg=(gt-gauge)/np.sqrt(2)
        ...
        if gauge_target=='paper':
            vl=np.sqrt(2*alpha*chi);vsl=np.sqrt(8/3);vst=np.sqrt(2)
...
if __name__=='__main__':
    ...
    out=[]
    for angle in [0.,.25*np.pi,.5*np.pi,.75*np.pi,np.pi]:
      for degree in [1,3]:
       for inner in [2,4,'volume']:
        for mode in ['zero_rate','radiation']:
         p=dict(n=16,h=.25,angle_y=angle,degree=degree,inner=inner,mode=mode)
         L,_=full_matrix(**p);eig=np.linalg.eigvals(L);z=.15*.25*eig;R=1+z+z*z/2+z*z*z/6
         r=dict(**p,max_real=float(max(eig.real)),rk3_radius=float(max(abs(R))))
         out.append(r);print(json.dumps(r),flush=True)
    Path('fourier-results.json').write_text(json.dumps(out,indent=2)+'\n')
