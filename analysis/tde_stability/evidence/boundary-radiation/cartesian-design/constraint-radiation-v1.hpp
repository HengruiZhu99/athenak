...
// Gamma_t use exactly the same active-cell derivative, including at block edges.
...
bool RadiationGeometry(const DvceArray5D<Real> &v, int m, int k, int j, int i,
  ...
  for (int a = 0; a < 3; ++a) {
    for (int b = a; b < 3; ++b) {
      ...
      for (int d = 0; d < 3; ++d) {
        dg[d][a][b] = dg[d][b][a] =
            RadiationDerivative(v,m,n,k,j,i,d,indcs,idx);
...
int ComputeConstraintRadiationResidual(
  ...
  (void)side;
  (void)normal_d;
  if (indcs.nx1 < 3 || indcs.nx2 < 3 || indcs.nx3 < 3) return 1;
  ...
  for (int a = 0; a < 3; ++a) {
    for (int b = a; b < 3; ++b) {
      ...
      for (int d = 0; d < 3; ++d) {
        drg[d][a][b] = drg[d][b][a] =
            RadiationDerivative(rhs,m,n,k,j,i,d,indcs,idx);
      }
    }
  }
  for (int a = 0; a < 3; ++a) {
