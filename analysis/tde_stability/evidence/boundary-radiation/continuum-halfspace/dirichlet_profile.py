...
import json
rt=bn*2*np.pi;ky=2*np.pi
rec,data=assess_schur(rt,ky,'radiation_dirichlet',True,True);U,D,T,B,R,ss,vv,Us,norms=data
coeff=np.linalg.solve(R,vv[-1].conj());coeff/=np.max(abs(U@coeff))
np.savez_compressed(ROOT/'dirichlet-halfspace-mode.npz',U=U,D=D,T=T,coeff=coeff,lambda_real=rt,ky=ky)
...
checks=[]
for x in [.2,.6,1.,rt,1.6,2.,3.]:checks.append({'lambda_real':x,**assess_schur(x,ky,'radiation_dirichlet',True)})
fit=optimize.minimize_scalar(lambda x:assess_schur(x,ky,'radiation_dirichlet',True)['sigma_min'],bounds=(.6,2),method='bounded')
result={'root_assessment':rec,'lambda_real':rt,'physical_profiles':profiles,'max_H':max(x['H'] for x in profiles),'max_M':max(x['M'] for x in profiles),'max_linear_electric_Weyl':max(x['E'] for x in profiles),'max_linear_magnetic_Weyl':max(x['B'] for x in profiles),'boundary_gauge_amplitudes':{k:profiles[-1][k] for k in ['lapse','shift']},'dirichlet_local_scan':checks,'dirichlet_minimum_0p6_to2':{'lambda_real':float(fit.x),'sigma_min':float(fit.fun)},'scope':'Alternative only replaces boundary lapse/shift rows. Positive-Re temporal range at one tangential frequency; not uniform stability proof.'}
(ROOT/'dirichlet-physical-validation.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k not in ['physical_profiles','dirichlet_local_scan']},indent=2))
