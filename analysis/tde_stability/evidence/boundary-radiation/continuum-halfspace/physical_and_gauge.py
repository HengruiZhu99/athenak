...
import json
m=np.load(ROOT/'validated-halfspace-mode.npz');U=m['U'];D=m['D'];T=m['T'];coeff=m['coeff'];rt=float(m['lambda_real']);ky=float(m['ky'])
profiles=[]
...
result={'physical_profiles':profiles,'max_H':max(x['H'] for x in profiles),'max_M':max(x['M'] for x in profiles),'max_linear_electric_Weyl':max(x['E'] for x in profiles),'max_linear_magnetic_Weyl':max(x['B'] for x in profiles),'boundary_gauge_amplitudes':{k:profiles[-1][k] for k in ['lapse','shift']},'dirichlet_local_scan':checks,'dirichlet_minimum_0p6_to2':{'lambda_real':float(fit.x),'sigma_min':float(fit.fun)},'scope':'Alternative only replaces boundary lapse/shift rows. Positive-Re temporal range at one tangential frequency; not uniform stability proof.'}
(ROOT/'physical-gauge-validation.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:v for k,v in result.items() if k not in ['physical_profiles','dirichlet_local_scan']},indent=2))
