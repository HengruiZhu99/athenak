...
op=Operator(str(ROOT/'candidate_validation'),binary=ROOT.parent/'athena-covariant-modehook',input_file=ROOT/'input.athinput',dt=.0375,overrides=('z4c/ccz4_covariant_sources=true','z4c/damp_lapse_scaled=true','z4c/damp_kappa1=.3'))
probe=Operator(str(ROOT/'candidate_physical_constraints'),binary=OLD/'athena-constraint-probe',input_file=OLD/'input-constraints.athinput')
...
for n,m in enumerate(json.loads((ROOT/'covariant_constant03-arnoldi32-results.json').read_text())['modes'][:2]):
 ...
 np.savez_compressed(ROOT/f'covariant03-mode{n}-physical-constraints.npz',H=c[1,4:20,4:20,4:20],M=c[4:7,4:20,4:20,4:20],Q=q,H_advanced=cr[1,4:20,4:20,4:20],M_advanced=cr[4:7,4:20,4:20,4:20],Q_advanced=qr)
 (ROOT/'candidate-validated-modes.json').write_text(json.dumps(results,indent=2)+'\n')
