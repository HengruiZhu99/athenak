...
Supports a serial run or a fixed MPI partition with exactly one block per rank.
...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "tst/regression"))
...
def validate(run, ranks, cycle=None):
    ...
    q = records[0]
    if ranks > 1:
        assert q["total"] == ranks and all(len(r["state"]) == 1 for r in records), \
            "MPI checker requires a uniform, one-block-per-rank partition"
    ...
    for b, loc in enumerate(locations):
        ...
        for k, j, i in np.argwhere(invalid)[:max(0, 16-len(samples))]:
            ...
            samples.append(dict(
                rank=b if ranks > 1 else 0, global_block=b,
