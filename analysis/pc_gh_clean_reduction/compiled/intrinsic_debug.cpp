...
int main(int argc,char **argv) {
  ...
  {
    Kokkos::View<double**> input("input",count,203), output("output",count,128), debug("debug",count,6);
    ...
    Kokkos::parallel_for("intrinsic RHS oracle",count,KOKKOS_LAMBDA(int r) {
      ...
      debug(r,0)=input(r,0);debug(r,1)=u[0];
      PointRHS(u,du,input(r,200),input(r,201),input(r,202),rhs);
      debug(r,2)=u[0];debug(r,3)=rhs[0];
      ConfigurationSources(u,input(r,201),f,ell);int pos=0;debug(r,4)=u[0];debug(r,5)=f[0];
    ...
    auto dh=Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(),debug);
    std::ofstream dbg(std::string(argv[3])+".debug");dbg<<std::setprecision(17);
    for(int r=0;r<count;++r) {for(int n=0;n<6;++n) dbg<<dh(r,n)<<" ";dbg<<"\n";}
