...
int main(int argc,char **argv) {
  ...
  Kokkos::initialize(argc,argv);
  {
    ...
    auto before=Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(),input);
    std::ofstream inspect(std::string(argv[3])+".input");inspect<<std::setprecision(17)<<"host "<<host(0,0)<<" device_before "<<before(0,0)<<"\n";
    ...
    auto after=Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(),input);
    inspect<<"device_after "<<after(0,0)<<"\n";
