...
import numpy as np
p=argparse.ArgumentParser(description=__doc__);p.add_argument('--cpu',type=Path,required=True);p.add_argument('--cuda',type=Path,required=True);a=p.parse_args()
...
cpu=np.loadtxt(a.cpu/'output.txt');gpu=np.loadtxt(a.cuda/'output.txt');reference=np.loadtxt(a.cpu/'expected.txt');meta=json.loads((a.cpu/'results.json').read_text())
