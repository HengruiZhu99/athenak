#!/usr/bin/env python3
"""Read-only validator for centered M=R0=1 trumpet AMR/SMR checkpoints.

Separate from the uniform checker. Supported format: little-endian doubles,
5-variable MHD plus 25 residual Z4c fields, per-rank files, validated dyadic AMR/SMR leaf mesh,
...
def check_supported(h):
    ...
    require(p['mesh_refinement']['refinement'] in ('static','adaptive'), 'Only 3D dyadic AMR/SMR supported')
    ...
    require(np.isfinite(h['costs']).all() and np.all(h['costs'] == 1), 'Only equal unit-cost partition supported')
...
def geometry(h, n, ns, lo, hi):
    ...
    # Dyadic leaves overlap iff one is an ancestor of another. This avoids
    # quadratic box comparisons for large adaptive checkpoint cohorts.
    for lx,ly,lz,level in location_set:
        for ancestor in range(h['root_level'],level):
            shift=level-ancestor
            require((lx>>shift,ly>>shift,lz>>shift,ancestor) not in location_set,
                    'Overlapping parent/child leaves')
    return blocks
...
def validate(run, ranks, cycle=None, exact_zero=False, audit_path=None, audit_case='mesh16_r1'):
    ...
    if audit_path:
        ...
        audit_match=True
    theta_regions={name:dict(volume=0.0,theta2=0.0,peak=None) for name in
                   ['horizon_interior','horizon_exterior','outer_face_256M','radial_sponge']}
    ...
    for rank,path in enumerate(files):
        ...
        for local in range(counts[rank]):
            ...
            if not invalid.any():
                ...
                rr=r[active_slice];theta=u[17][active_slice]  # I_Z4C_THETA in src/z4c/z4c.hpp
                weight=np.sqrt(det[active_slice])*chi[active_slice]**(-1.5)*float(np.prod(b['dx_M']))
                xx0,yy0,zz0=x[active_slice],y[active_slice],z[active_slice]
                face=np.minimum.reduce([xx0-lo[0],hi[0]-xx0,yy0-lo[1],hi[1]-yy0,zz0-lo[2],hi[2]-zz0])
                start=float(h['params']['problem'].get('outer_sponge_start_radius',512))
                masks={'horizon_interior':rr<=1,'horizon_exterior':rr>1,
                       'outer_face_256M':(face<=256)&(rr>1),'radial_sponge':(rr>start)&(rr>1)}
                for name,mask in masks.items():
                    if not mask.any():continue
                    group=theta_regions[name];group['volume']+=float(weight[mask].sum())
                    group['theta2']+=float((theta[mask]**2*weight[mask]).sum())
                    magnitude=np.where(mask,np.abs(theta),-1);at=np.unravel_index(magnitude.argmax(),theta.shape)
                    value=float(magnitude[at])
                    if group['peak'] is None or value>group['peak']['absolute_Theta']:
                        kk,jj,ii=map(int,at)
                        group['peak']={'absolute_Theta':value,'signed_Theta':float(theta[at]),'rank':rank,'gid':gid,
                                       'relative_level':b['relative_level'],
                                       'xyz_M':[float(axes[0][ii+ng]),float(axes[1][jj+ng]),float(axes[2][kk+ng])]}
        sha=hashlib.sha256()
