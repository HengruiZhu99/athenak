...
: "${PREVIOUS_RUN_ROOT:?completed N256 record segment}"
...
test ! -e "${RUN_ROOT}"
test -s "${PREVIOUS_RUN_ROOT}/n256_native_authority.jsonl"
...
cp "${PREVIOUS_RUN_ROOT}/n256_native_authority.jsonl" "${RUN_ROOT}/"
cp "${PREVIOUS_RUN_ROOT}/brill_global_128x32.coefficients" "${RUN_ROOT}/"
cp "${PREVIOUS_RUN_ROOT}/brill_vc_figure3.athinput" "${RUN_ROOT}/"
...
env | LC_ALL=C sort > "${RUN_ROOT}/evidence/environment.txt"
{
  ...
  sha256sum "${athena}" "${build_root}/CMakeCache.txt" \
    "${restart_file}" "${RUN_ROOT}/n256_native_authority.jsonl"
  module list
  nvidia-smi -L
...
command=(srun --nodes=1 --ntasks=1 --ntasks-per-node=1 --cpus-per-task=32
  ...
  mesh_refinement/amr_history_file=n256_native_authority.jsonl
  time/tlim="${RUN_TLIM}" time/nlim=-1
  job/basename=n256_native_record
  problem/brill_global_coefficients_file=brill_global_128x32.coefficients
  problem/constraint_summary_file=n256_native_record-constraints.dat)
