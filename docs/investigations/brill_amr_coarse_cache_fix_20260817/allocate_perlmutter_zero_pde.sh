...
job=cartoon-brill-cache-ab651-v5
...
salloc --account="${account}" --job-name="${job}" --qos=shared_interactive \
  --constraint='gpu&hbm40g' --ntasks=2 --ntasks-per-node=2 \
  --cpus-per-task=16 --gpus-per-node=1 --time=02:00:00 \
  bash "${root}/run_perlmutter_zero_pde.sh" \
...
if [[ ${#ids[@]} -eq 1 && ${ids[0]} =~ ^[0-9]+$ ]]; then
  ...
  for _ in {1..60}; do
    state=$(sacct -n -X -P -j "${id}" -o State | sed '/^[[:space:]]*$/d' | head -1 | cut -d'|' -f1)
    case "${state}" in
