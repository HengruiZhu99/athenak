...
campaign=Path('/pscratch/sd/h/hzhu/lapse-bisection-t200-corner-fixed-20260910')
if (campaign/'state.json').exists():raise RuntimeError('Campaign already has state')
verified=json.loads((root/'early-lapse/validation/verification.json').read_text())
exe=root/'early-lapse/build/src/athena'
if verified['status']!='PASS' or sha(exe)!=verified['new_executable_sha256']:raise RuntimeError('Executable qualification mismatch')
for name,digest in json.loads((root/'early-lapse-source-files.json').read_text()).items():
 if sha(root/'early-lapse/source'/name)!=digest:raise RuntimeError('Compiled source differs: '+name)
base=root/'early-lapse/baseline';base.mkdir()
...
(base/'source-base.txt').write_text('a8e80b970ea1a0058a35adf91b8981c225195933\n');(base/'source.patch').write_text('')
shutil.copyfile(root/'early-lapse-source-files.json',base/'source-files.json')
...
evidence=json.loads((root/'early-lapse/historical-endpoints/evidence.json').read_text())
...
q=dict(executable_sha256=sha(exe),source_sha='a8e80b970ea1a0058a35adf91b8981c225195933',approved_configuration={'boundary_rhs':'full_constraint_bjorhus','extrap_order':2,'vc_single_rank_device_sync':True},runtime_stop_validation=verified,legacy_endpoint_evidence=evidence,boundary_evidence={'fresh_sub_job':'58135543','fresh_super_clean_segment_job':'58134624','t90_to200_job':'58134571','limitation':'Empirical suppression of previous corner instability; not spatial convergence qualification.'})
