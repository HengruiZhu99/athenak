...
case=root/'fresh_sub_cpbc_linear'
case.mkdir(exist_ok=False)
(case/'amplitude.txt').write_text('-0.047\n')
...
(case/'provenance.json').write_text(json.dumps(dict(baseline=str(base),exe_sha256=sha(base/'athena.history_extrema'),input_sha256=sha(case/'input.athinput'),purpose='Fresh A=-0.047 N256 boundary qualification, not a bisection midpoint',changes=['full_constraint_bjorhus','extrap_order=2','device sync','full-domain constraint diagnostics','curvature output dt5']),indent=2))
print(case)
