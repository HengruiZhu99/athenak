...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--baseline',required=True,type=Path);a=ap.parse_args();base=a.baseline.resolve(strict=True)
    with (R/'controller.lock').open('w') as lock:
        ...
        def run_case(amplitude,name):
            ...
            record=dict(amplitude=str(amplitude),classification=result,final=h[-1],directory=str(case),job_id=(case/'job-id.txt').read_text().strip(),checkpoint=str(checkpoints[-1]),checkpoint_sha256=sha(checkpoints[-1]),coefficient_sha256=sha(case/'initial.coefficients'),input_sha256=sha(case/'input.athinput'),finished=time.time())
            save(case/'result.json',record);state['completed'].append(record);state.update(status='CLASSIFIED',active=None);save(R/'state.json',state)
