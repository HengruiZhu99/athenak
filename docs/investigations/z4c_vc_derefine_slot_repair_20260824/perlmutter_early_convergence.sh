...
# Retain the complete writer census only at the already-verified event-3 cycle.
# This is a default-off diagnostic and does not change the production state.
...
command=(srun --nodes=1 --ntasks=1 --ntasks-per-node=1 --cpus-per-task=32
  ...
  -i brill_vc_figure3.athinput -t 00:40:00
  ...
  time/tlim=2.5 time/nlim=-1 job/basename="${CASE_LABEL}_early"
  problem/brill_global_coefficients_file=brill_global_128x32.coefficients
  problem/constraint_summary_file="${CASE_LABEL}_early-constraints.dat")
