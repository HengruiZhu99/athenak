...
coordinate time `0.2979919496568637 M`.  It uses source commit
`d2596707e808aea7ec6167df937d71dc4dbe429e`, exact VC injection, O4 bulk
...
allocations.  All three executions returned zero.
...
- A156 to A157 changes those variables' full-block hashes while their strict
  ...
  interior writer.
- A159 to A16, A16 to A17, and A17 to A18 do not change the active Z4c state.
...
Observation: the predicted A5/A6 corruption is absent after the repair, the
active state is consistent through A16, and the first subsequent evolution is
finite in all three bounded runs.
...
Primary evidence is under `evidence/perlmutter/event3_*`; the compact plot is
