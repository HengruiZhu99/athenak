...
[ "$(cat "$root/build-befad6a4.status")" = 0 ]
[ "$(git -C "$root/source" rev-parse --short=8 HEAD)" = befad6a4 ]
out="$root/gpu_validation_befad6a4"
mkdir "$out"
...
for variant in adaptive coarse-group global-gauge cpbc rollback stage-limits; do
  args=(--adaptive)
  case "$variant" in
    ...
    stage-limits) args+=(--stage-limits);;
  esac
