...
### Corrected-output benchmark running with verified histories
Output cadence CUDA build959300 finishedstatus0 and source/executablehashchecks passed. Pinned at pinned-a26f002f/athena with884f5170 and outputcadence patches (embedded38c1fab4). Fresh tools endpoint-tools-a26f002f and manifest brill-endpoint-a26f002f. Job58211022 nid008237,45minute shared_interactive oneA10080GB, session85596. GPU restart-output cadence regression passed before actualcases. Verified at2m07s that legacy_sync evolvesactualcheckpoint and writes lapse200.z4c.user.hst. Prior58210606 cancellation remains invalid-output attempt. Currentcase classical/subcycled will followserially; endpointscientificcomparison stillpending. Archive manifest/regression/slurm/scripts in brill-cadence-rerun.

### Actual legacy control reproduces archived endpoint diagnostics
Job58211022 legacy_sync completed at62.20599406231406 in351.323588s including startup/output. minLapse=.017361930148278765, maxAbsKret=259724.66624038288, C-norm2=131.3371573622191, axisLapse=.510622988950541, nmb9155,maxRefLev18 all EXACTLY match archived final history atstoredprecision. Downloaded completedhistory/log/result and asserted equality locally; endpoint-check.json records scope. This validates the baseline checkpoint continuation, not subcycling or original physical collapse. Job confirmed live6m24s starting classical_sync; subcycled follows.

### Classical control completed; subcycled case started
Job58211022 classical_sync reachedsameendpoint in349.834553s, nmb9155/maxlevel18same as archived. Relative differences: lapse3.84e-13, maxAbsKret-9.43e-10, Cnorm1.32e-12, axial lapse2.50e-13. Archived completedhist/log/result/endpointcheck. At12m48s jobverifiedlive starting subcycled actualcheckpoint. No subcycling success/speedup conclusion yet. Read earlier checkpointheaders:63t62.034604/cycle118310,64t62.177782/cycle125328,65t62.200215/cycle131469,66current; inventory saved. Preparation now accepts explicitcheckpoint-index so later reproduction can cover largerinterval; activejobtools unchanged.

### Actual subcycling production gate failed before first interval
Job58211022 terminal FAILED1:0 at13m44s. Subcycled case aborted134 with subcycling::CorrectorFailure (pass budget exhausted); no accepted interval rows (CSV0lines due bufferedheader). Legacy andclassical completedsuccessfully. Archived failed stdout/stderr/result. Do not mark reproduction or speedup successful. Existing error omits endpoint/history/gauge normalizedresiduals and retry dt; next action instrument per-pass/attempt diagnostics, reproduce isolated failure, identify culprit without relaxing acceptance blindly. Original production fields not modified.

### Add invariant corrector tracing and start CUDA diagnostic build
ATHENA_SUBCYCLE_TRACE enables per-attempt dt/ratio and per-pass normalized endpoint/history/gauge-feedback changes, worst history level/tick, classified retryreason. No tolerance or algorithm change. CPU production-gauge fixedtime test passes and all6 endpoint fieldhashes exactlymatch pretrace stage-gauge evidence; stage stability/retry test passes with tracing. Source-only patch uploaded/applied to isolated remote existingbase+patches. Diagnostic build PID1051167 verifiedlive, build-corrector-trace.log/status underbuild-corrector-trace. Next boundedGPUrun should reproduce firstinterval only using fresh isolated AMRprefix and traceenv, no rerun of successful controls; production correctorfailure still unresolved.
