...
prior_evidence=${campaign_root}/evidence/phase5-brill-initial-analysis-recovery1
evidence_root=${campaign_root}/evidence/phase5-brill-initial-analysis-recovery2
...
test "$(cat "${prior_evidence}/exit-status.txt")" = 0
test ! -e "${evidence_root}"
for resolution in 128 256 512; do
  test -f "${run_root}/N${resolution}/rhs.rank000000.csv.gz"
...
for comparison in summary["common_node_field_comparisons"]:
    assert comparison["direct_initialized_field_linf"] == 0.0
assert all(math.isfinite(value)
           for family in summary["common_node_difference_orders"].values()
           for item in family.values() for value in item.values()
           if value is not None)
PY

find "${run_root}" -type f -print0 | sort -z | xargs -0 -r sha256sum \
  > "${evidence_root}/run-products.sha256"
sha256sum "${prior_evidence}/SHA256SUMS" \
  > "${evidence_root}/prior-analysis-manifest.sha256"
printf '%s\n' \
  'DIRECT_VC_BRILL_INITIAL_DATA_COMMON_NODE_DIFFERENCES_ANALYZED' \
