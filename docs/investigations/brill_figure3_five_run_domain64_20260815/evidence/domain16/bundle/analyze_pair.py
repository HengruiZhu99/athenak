#!/usr/bin/env python3
"""Summarize the level-20 zero-constraint-damping control without mutation."""
...
def comparison_result(cases: list[dict]) -> dict:
    assert [case["name"] for case in cases] == [
        "fixed_eta2_tau1_kappa1_l20_nocd_ko002",
        "fixed_eta2_tau1_kappa1_l20_nocd_ko05",
        "zero_shift_tau1_kappa1_l20_nocd_ko05",
    ]
    ...
    assert [case["ko_dissipation"] for case in cases] == [0.02, 0.5, 0.5]
    ...
    return {
        "schema": "athenak_brill_l20_tau1_nocd_ko_shift_trio_mpi4_v1",
        "source_commit": "2a8ad80e02279769a99fe279b7a33516bc6c8d0d",
        "exact_common_changes": {
            "mesh_refinement/num_levels": 21,
            "time/nmb_total_limit": 8192,
            "z4c/shift_eta_max_K": False,
            "z4c_amr/max_ref_lev": 20,
        },
        "exact_difference_from_predecessor_tau1": {
            "z4c/damp_kappa1": [0.02, 0.0],
            "z4c/target_kappa1": [0.02, 0.0],
            "z4c/damp_kappa1_max_K": [True, False],
        },
...
def self_test() -> None:
    with tempfile.TemporaryDirectory() as tmp_name:
        ...
        first = dict(result)
        first.update(
            name="fixed_eta2_tau1_kappa1_l20_nocd_ko002",
            tau=1.0,
            kappa=1.0,
            ko_dissipation=0.02,
            rank_bindings=[{"binding_verified": True}] * 4,
        )
        second = dict(first)
        second.update(
            name="fixed_eta2_tau1_kappa1_l20_nocd_ko05",
            ko_dissipation=0.5,
        )
        third = dict(second)
        third.update(
            name="zero_shift_tau1_kappa1_l20_nocd_ko05",
            shift_condition="zero_shift",
        )
        comparison = comparison_result([first, second, third])
        ...
        try:
            comparison_result([first, mutated, third])
...
def main() -> None:
    ...
    comparison.add_argument("--case-results", type=Path, nargs=3, required=True)
