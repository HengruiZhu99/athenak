...
salloc --account="${COLLAPSE_ACCOUNT}" --job-name="${EXPECTED_JOB_NAME}" \
  --qos=shared_interactive --constraint='gpu&hbm80g' --nodes=1 --ntasks=1 \
  --ntasks-per-node=1 --cpus-per-task=32 --gpus-per-node=1 --time=04:00:00 \
  bash "${bundle_dir}/run_allocation.sh" > "${root}/allocation/allocation.stdout" \
...
if [[ ${#job_ids[@]} -eq 1 && ${job_ids[0]} =~ ^[0-9]+$ ]]; then
  ...
  for _ in $(seq 1 120); do
    ...
    state=$(awk -F'|' -v id="${job_id}" '$1==id {print $3}' \
      "${root}/allocation/sacct-current.psv")
    case "${state}" in COMPLETED|FAILED|CANCELLED|TIMEOUT|OUT_OF_MEMORY) break ;; esac
    sleep 2
  done
  mv "${root}/allocation/sacct-current.psv" "${root}/allocation/sacct-settled.psv"
fi
