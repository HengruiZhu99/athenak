...
run_root=${campaign_root}/runs/outer-onesided-linear-ghost-n512
evidence_root=${campaign_root}/evidence/outer-onesided-linear-ghost-n512
...
test -n "${SLURM_JOB_ID:-}" && test "${SLURM_GPUS:-0}" -eq 1
test "$(git -C "${source_root}" rev-parse --short=8 HEAD)" = ba7daebc
...
"${athena}" -i "${input}" -d "${run_root}" \
  job/basename=outer_onesided_linear_ghost_N512 \
...
history=${run_root}/outer_onesided_linear_ghost_N512.z4c.user.hst
test -s "${history}"
tail -1 "${history}" > "${evidence_root}/terminal-history-row.txt"
printf '%s\n' OUTER_ONESIDED_LINEAR_GHOST_N512_REACHED_T5 \
