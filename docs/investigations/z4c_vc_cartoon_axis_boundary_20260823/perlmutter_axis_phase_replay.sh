...
run_root=${campaign_root}/runs/axis-phase-replay
evidence_root=${campaign_root}/evidence/axis-phase-replay
...
ctest --test-dir "${build_root}" --output-on-failure \
  -R 'z4c_vc_cartoon_(derivatives_o[246]|axis_(scalar|vector|tensor|adm|constraint|rhs_regularity))|z4c_cartoon_production_kernels_cuda_required' \
...
for resolution in 128 256 512; do
  for tag in 0125 0250 0500; do
    exact=${checkpoint_root}/N${resolution}/tau${tag}/exact
    ...
    diagnostic=${run_root}/N${resolution}/tau${tag}
    mkdir -p "${diagnostic}"
    (
      cd "${diagnostic}"
      "${athena}" -r "${restart}" -d "${diagnostic}" \
        job/basename="axis_phase_N${resolution}_tau${tag}" \
        time/nlim="$((cycle + 1))" \
        time/tlim="$(${python_bin} - "${target}" <<'PY'
import sys
...
PY
        ...
        > "${evidence_root}/N${resolution}.tau${tag}.stdout.log" \
        2> "${evidence_root}/N${resolution}.tau${tag}.stderr.log"
    )
    ...
    printf '%s %s %.17g %s\n' "${resolution}" "${tag}" "${target}" "${cycle}" \
      >> "${evidence_root}/checkpoint-inventory.txt"
  done
done

printf '%s\n' AXIS_PHASE_AND_TERM_PROVENANCE_CAPTURED \
