...
evidence_root=${campaign_root}/evidence/fixed-grid-gate-repaired
run_root=${campaign_root}/runs/fixed-grid-gate-repaired
required_commit=2b8fe7fd
...
test -f "${coefficient}" && test -f "${input}"
grep -Eq '^extrap_order[[:space:]]*=[[:space:]]*4$' "${input}"
test ! -e "${evidence_root}" && test ! -e "${run_root}"
...
for resolution in 128 256 512; do
  ...
  basename=z4c_vc_axis_fixed_repaired_N${resolution}
  mkdir -p "${run}"
  start=$(date +%s)
...
printf '%s\n' 'REPAIRED_FIXED_GRID_BRILL_N128_N256_N512_REACHED_AXIS_TAU_3' \
