...
More explicitly, concatenate the independent stored components of E into e and
write the displayed subsidiary system as `e_t=beta^k partial_k e+(M-lambda I)e`.
Here M contains the derivative-index action of `partial_i beta^j`, the map
`b_i^k -> b_i^k G_k^A`, the configuration Jacobian of F, and the intrinsic N map.
All of these coefficients have already been specified above; no Einstein or GH
constraint is set to zero to obtain this equation. On a fixed coordinate region,
direct integration by parts gives

\[
\frac{d}{dt}\int |e|^2 d^3x
=\int_{\partial\Omega}(\beta\cdot n)|e|^2dS
+2\int e^T[\operatorname{Sym}M-(\lambda+\tfrac12\partial_k\beta^k)I]e\,d^3x.
\]

With controlled incoming boundary data, a sufficient instantaneous L2 decay
condition is `lambda > sup lambda_max(Sym M) - div(beta)/2`, where the supremum
applies to the complete scalar expression. This is a conditional estimate on
the evolving background, not an assertion that the tested lambda=1 satisfies it.
Higher derivative estimates also contain coefficient-gradient commutators. The
frozen spectral check below permits transient amplification and is weaker than
this monotone-energy condition. These distinctions matter when interpreting
growing pointwise constraints in the puncture screens.
...
The preceding evidence is AppleClang/Kokkos Serial zero-step and symbolic work.
After the user restored SSH access, the same production oracle passed on CUDA,
followed by exact Minkowski, compact-pulse and shifted-wave evolutions. The
dated qualification log records their measured rates and convergence, including
coarse-resolution limitations. These earlier gates do not qualify a puncture
scheme. The saved hard-Q correction and old 73.7999M metric-positivity failure
remain relevant controls. No result here attributes that failure to projection.
