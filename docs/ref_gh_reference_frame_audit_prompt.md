...
The branch deliberately stops after adding and locally testing the exact
stationary n=2 1+log trumpet reference and common ADM/history diagnostics.  It
...
6. Derive the stationary trumpet coframe, inverse frame, connection, structure
   coefficients, and their derivatives.  Verify the exact regular state
   `Psi=eta`, `Pi=Phi=0` and explain the observed initial RHS near `1e-9`.
