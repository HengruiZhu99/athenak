...
boundary.  One supported gauge path is the code's
...
The \texttt{standard\_subtract} gauge is also supported. It applies
Equation~\eqref{eq:residual} to the ordinary gauge equations as well as the
geometric equations. Its background term is a prescribed source. In all the
finite-residual formulas below, set
\[
 b_b=b_f,\qquad
 L=\alpha_{\rm full}\left(f_{\rm oplog}f_{\rm harmonicf}
                  +f_{\rm harmonic}\alpha_{\rm full}\right).
\]
Thus $b_b$ denotes the selected gauge advection coefficient; it equals the
analytic-background shift only for \texttt{background\_adapted}. The same
selection applies to the full vector advection in the tangential-principal
boundary datum. The \texttt{residual\_lapse\_f} multiplier belongs only to the
adapted gauge and is not applied to standard subtraction. The lower-order
terms of the two gauges differ even when their zero-residual principal
symbols agree. Matching the principal boundary symbol does not establish
long-time stability of either gauge.
...
the adapted gauge driver and background-shift advection identified explicitly
