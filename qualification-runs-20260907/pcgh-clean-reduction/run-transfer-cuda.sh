...
trap 'printf "%s\n" "$?" > test.exit' EXIT
while [ ! -f build.exit ]; do
  if ! kill -0 385334 2>/dev/null; then
...
p=pathlib.Path('source'); manifest=json.load(open('source-manifest.json'))
...
for ranks in 1 2; do
  for topology in uniform smr; do
    nvidia-smi --query-gpu=name,memory.total,memory.used,memory.free,utilization.gpu --format=csv,noheader > "gpu-before-${ranks}-${topology}.csv"
    python3 - "$ranks" "$topology" <<'PY'
import pathlib,sys
row=pathlib.Path(f'gpu-before-{sys.argv[1]}-{sys.argv[2]}.csv').read_text().strip().split(',')
assert len(row)==5 and int(row[3].split()[0])>=4096, 'insufficient GPU memory for bounded fixtures'
PY
    label="cuda-${ranks}rank-${topology}-001"
    extra=()
