...
void Final(ParameterInput *pin, Mesh *mesh) {
  ...
  for (int repeat=0; repeat<3; ++repeat) {
    ...
    for (int m=0; m<nmb; ++m) {
        ...
        for (int j=0; j<state.extent_int(3); ++j) {
          for (int i=0; i<state.extent_int(4); ++i) {
            ...
            for (int n=0; n<PC::npcgh; ++n) {
              ...
              target+=.001*(n-PC::I_P1+1)*(smooth?std::sin(phase_at(rk,rj,ri)+.13*n):1);
              ...
              ghost_component_before[n-PC::I_P1]=std::max(
                  ghost_component_before[n-PC::I_P1],std::abs(delta_before));
              ghost_component_after[n-PC::I_P1]=std::max(
                  ghost_component_after[n-PC::I_P1],std::abs(delta_after));
      ...
      Real curl_tensor_before[33]={}, curl_tensor_after[33]={};
      ...
      for (int k=ind.ks; k<=ind.ke; ++k) for (int j=ind.js; j<=ind.je; ++j) {
        for (int i=ind.is; i<=ind.ie; ++i) for (int family=0; family<11; ++family) {
          for (int a=0; a<3; ++a) for (int b=a+1; b<3; ++b) {
            ...
            curl_tensor_before[tensor]=std::max(curl_tensor_before[tensor],std::abs(cb));
            curl_tensor_after[tensor]=std::max(curl_tensor_after[tensor],std::abs(ca));
      ...
      auto array=[&](const char *name,const Real *values,int count=11) {
        out << ",\"" << name << "\":[";
        for (int q=0; q<count; ++q) {if (q) out << ',';out << values[q];}
        out << ']';
      };
      array("ghost_component_error_before_max",ghost_component_before,33);
      array("ghost_component_error_after_max",ghost_component_after,33);
      array("curl_tensor_error_before_max",curl_tensor_before,33);
      array("curl_tensor_error_after_max",curl_tensor_after,33);
