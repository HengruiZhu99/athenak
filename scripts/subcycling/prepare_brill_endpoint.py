...
with histories[0].open() as f:
    for line in f:
        if line.strip() and not line.startswith('#'):last=line
...
manifest=dict(checkpoint=str(checkpoint),checkpoint_sha256=sha(checkpoint),
    ...
    original_final_history_row=last.strip(),exe=str(exe),exe_sha256=sha(exe),cases=[])
for name,ratio in [('classical_sync',0),('subcycled',a.ratio)]:
    case=output/name;case.mkdir();(case/'amr_history.jsonl').write_bytes(prefix)
    text=base
    parameters=[('time','integrator','rk4_classical'),('time','nlim',-1),('time','tlim',end),
    ...
    manifest['cases'].append(dict(name=name,directory=str(case),ratio=ratio,
