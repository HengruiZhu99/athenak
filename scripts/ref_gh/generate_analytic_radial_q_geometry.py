...
def emit_gauge(output: Path,
               fields: Dict[str, Dict[Index, sp.Expr]] | None = None) -> None:
    if fields is None:
...
def hot_reference_components(
        fields: Dict[str, Dict[Index, sp.Expr]]) -> List[sp.Expr]:
    """Return the exact 24+96+21 production-cache storage ordering.

    These are the stored, symmetry-reduced values, not arbitrary tensor
    components.  Their order exactly matches ReferenceHotComponent and the
    established generic ReferenceCachePoint accessors.
    """
    values: List[sp.Expr] = []
    antisymmetric_pairs = tuple(itertools.combinations(range(4), 2))
    for capital_a, capital_b in antisymmetric_pairs:
        for capital_c in range(4):
            values.append(fields["spin"][capital_a, capital_b, capital_c])
    for capital_d in range(4):
        for capital_a, capital_b in antisymmetric_pairs:
            for capital_c in range(4):
                values.append(fields["spin_derivative"][
                    capital_d, capital_a, capital_b, capital_c])
    bivectors = antisymmetric_pairs
    for first_pair in range(6):
        capital_a, capital_b = bivectors[first_pair]
        eta_a = -1 if capital_a == 0 else 1
        for second_pair in range(first_pair, 6):
            capital_c, capital_d = bivectors[second_pair]
            values.append(eta_a*fields["riemann_frame"][
                capital_a, capital_b, capital_c, capital_d])
    assert len(values) == 141
    return values


def emit_hot_reference(output: Path,
                       fields: Dict[str, Dict[Index, sp.Expr]] | None = None
                       ) -> None:
    """Emit direct component expressions for the 141-Real hot cache.

    CSE is intentionally local to one stored component.  Production launches
    a flat cell-times-component kernel, so a work item never owns the full
    spin/dspin/Riemann tensor or the generic metric-jet workspace.
    """
    if fields is None:
        fields = build_geometry()
    values = hot_reference_components(fields)
    generator_sha = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    lines = [
        "// Generated by scripts/ref_gh/generate_analytic_radial_q_geometry.py.",
        "// Do not edit by hand.",
    ...
    lines.extend(compact_scalar_declarations())
    lines.append("  switch (component) {")
    radial_x0, radial_x1, radial_x2 = sp.symbols("X0:3", real=True)
    radial_radius = sp.Symbol("radius", positive=True)
    radial_relation = radial_x2**2 - (
        radial_radius**2 - radial_x0**2 - radial_x1**2)
    for component, expression in enumerate(values):
        # The raw symbolic Cartan construction contains large cancelling
        # rational sums.  Flattening those sums directly loses digits near the
        # puncture.  Exact rational cancellation followed by conservative
        # factor extraction produces the same expression with the singular
        # powers exposed and a stable binary64 evaluation order.
        expression = sp.cancel(expression)
        numerator, denominator = sp.fraction(expression)
        if denominator.has(radial_x2):
            raise RuntimeError(
                f"hot component {component} has coordinate-dependent denominator")
        # radius is an input computed from the same displacement, so reduce
        # exact Cartesian polynomials modulo X^2-r^2 before emitting floating
        # arithmetic.  Without this step, near-puncture expressions subtract
        # O(r^-n) terms whose symbolic sum is identically zero.
        numerator = sp.rem(numerator, radial_relation, radial_x2)
        expression = sp.factor_terms(sp.cancel(numerator/denominator))
        if expression == 0:
            lines.append(f"    case {component}: return 0.0;")
            continue
        replacements, reduced = sp.cse(
            [expression], symbols=sp.numbered_symbols(
                f"ref_hot_{component}_cse_"), order="canonical")
        if replacements:
            lines.append(f"    case {component}: {{")
            ...
            lines.append(f"    case {component}: return {cpp(reduced[0])};")
    lines.extend([
        "    default: return NAN;",
        "  }",
        "}",
...
def main() -> None:
    ...
    parser.add_argument("--hot-reference-output", type=Path)
    arguments = parser.parse_args()
    emit(arguments.output)
    generated_fields = None
    if (arguments.gauge_output is not None
            or arguments.source_output is not None
            or arguments.hot_reference_output is not None):
        generated_fields = build_geometry()
    if arguments.gauge_output is not None:
        emit_gauge(arguments.gauge_output, generated_fields)
    if arguments.source_output is not None:
        emit_source(arguments.source_output, generated_fields)
    if arguments.hot_reference_output is not None:
        emit_hot_reference(arguments.hot_reference_output, generated_fields)
