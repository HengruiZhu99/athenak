...
failed_run=${root}/run/n256-replay-provenance-v2
run=${root}/run/n256-replay-provenance-v3
...
grep -F 'Kokkos::deep_copy with no available copy mechanism' \
  "${failed_run}/run.log" >/dev/null
test -s "${evidence}/v2-diagnostic-layout-failure/binding.sha256"
test ! -e "${run}"
mkdir -p "${run}/bindings" "${run}/parse-smoke" "${evidence}/layout-fix"
...
finish() {
  ...
  git -C "${source_root}" status --porcelain=v1 > "${evidence}/source-status.final-v3"
  git -C "${source_root}/kokkos" status --porcelain=v1 > \
    "${evidence}/kokkos-status.final-v3"
...
sha256sum "${source_root}/src/z4c/chi_parent_provenance.cpp" > \
  "${evidence}/layout-fix/source.sha256"
cmake --build "${build_root}" --parallel 8 --target athena > \
  "${evidence}/layout-fix/build.log" 2> "${evidence}/layout-fix/build.err"
ctest --test-dir "${build_root}" --output-on-failure \
  ...
  > "${evidence}/layout-fix/focused-tests.log" 2>&1
grep -F '100% tests passed, 0 tests failed out of 5' \
  "${evidence}/layout-fix/focused-tests.log" >/dev/null
sha256sum "${build_root}/src/athena" "${build_root}/CMakeCache.txt" > \
  "${evidence}/layout-fix/build-products.sha256"
printf 'CUDA_LAYOUT_FIX_BUILD_AND_TESTS_PASS\n' > \
  "${evidence}/layout-fix/status.txt"

parse_command=("${build_root}/src/athena" -r "${restart}" -n
  -d "${run}" job/basename=N256-replay-provenance-v3
  problem/brill_global_coefficients_file="${coeff}"
  problem/constraint_summary_file=N256-replay-provenance-v3-constraints.dat
