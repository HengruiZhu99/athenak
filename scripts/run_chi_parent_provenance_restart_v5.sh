...
run=${root}/run/n256-replay-provenance-v5
...
grep -Fx 'OWNER_CANCELLED_FOR_CONTIGUOUS_CHI_PACK_OPTIMIZATION' \
  "${evidence}/v4-partial-diagnostic/disposition.txt" >/dev/null
test -s "${evidence}/v4-partial-diagnostic/binding.sha256"
test ! -e "${run}"
mkdir -p "${run}/bindings" "${run}/parse-smoke" "${evidence}/packed-chi-fix"
...
finish() {
  ...
  git -C "${source_root}" status --porcelain=v1 > "${evidence}/source-status.final-v5"
  git -C "${source_root}/kokkos" status --porcelain=v1 > \
    "${evidence}/kokkos-status.final-v5"
...
sha256sum "${source_root}/src/z4c/chi_parent_provenance.cpp" > \
  "${evidence}/packed-chi-fix/source.sha256"
cmake --build "${build_root}" --parallel 8 --target athena > \
  "${evidence}/packed-chi-fix/build.log" 2> \
  "${evidence}/packed-chi-fix/build.err"
ctest --test-dir "${build_root}" --output-on-failure \
  ...
  > "${evidence}/packed-chi-fix/focused-tests.log" 2>&1
grep -F '100% tests passed, 0 tests failed out of 5' \
  "${evidence}/packed-chi-fix/focused-tests.log" >/dev/null
sha256sum "${build_root}/src/athena" "${build_root}/CMakeCache.txt" > \
  "${evidence}/packed-chi-fix/build-products.sha256"
printf 'CUDA_PACKED_CHI_BUILD_AND_TESTS_PASS\n' > \
  "${evidence}/packed-chi-fix/status.txt"

parse_command=("${build_root}/src/athena" -r "${restart}" -n
  -d "${run}" job/basename=N256-replay-provenance-v5
  problem/brill_global_coefficients_file="${coeff}"
  problem/constraint_summary_file=N256-replay-provenance-v5-constraints.dat
